...
# UNQ_C1
# pred = (1/(2*math.pi*var)**(1/2))*np.exp((-1)*((np.subtract(p_val-mu))**2)*(2*var))
...
def select_threshold(y_val, p_val): 
    ...
    for epsilon in np.arange(min(p_val), max(p_val), step_size):
        ...
        ### START CODE HERE ###
        true_pos = np.sum((y_val==1) & (predictions==1))
        #print("True POS",true_pos)
        
        false_pos = np.sum((predictions == 1) & (y_val == 0))
        #print("FALSE POS",false_pos)
        false_neg= np.sum((y_val==1) & (predictions==0))
