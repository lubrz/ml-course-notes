...
def cofi_cost_func(X, W, b, Y, R, lambda_):
    ...
    for j in range(nu):
        ...
        for i in range(nm):
            ...
            #J += (1/2)*(R[i,j]*((np.dot(w],X[i,:])+b_j)-Y[i,j]))**2

    J=J/2
    J += (lambda_/2) * (np.sum(np.square(W)) + np.sum(np.square(X)))
    """
    m = np.dot(X,np.transpose(W))
    
    result = np.add(m,b)
    
    diff = np.subtract(result,Y)
    
    diff = np.multiply(diff,Y)
        
    J = diff**2
    
    J = np.divide(J,(1/2))
        
    W_l = (lambda_/2.0) * np.dot(W,np.transpose(W))**2
    
    W_l = np.multiply(W_l,R)
    
    X_l = (lambda_/2.0) * np.dot(X,np.transpose(X))**2
    
    X_l = np.multiply(X_l,R)
